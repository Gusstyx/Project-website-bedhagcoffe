...
function renderPredictionChart(histori, prediksi) {
  ...
  const borderColors = [...histori.map(() => "rgba(75, 192, 192, 1)"), ...prediksi.map(() => "rgba(255, 159, 64, 1)")];
...
function runPrediction() {
  ...
  fetch("/api/prediksi/proses", {
    ...
    .then((json) => {
      ...
      console.log("Prediksi data:", data); // Debugging: lihat data yang diterima
...
function renderPredictionSummary(data) {
  ...
  document.getElementById("prediksiMingguDepan").textContent = fp ? `${fp.prediksi_terjual} kg` : "-";
  // Sisa stok saat ini (stok akhir terakhir)
  document.getElementById("sisaStokSaatIni").textContent = data.stok_akhir_terakhir != null ? `${data.stok_akhir_terakhir} kg` : "-";
